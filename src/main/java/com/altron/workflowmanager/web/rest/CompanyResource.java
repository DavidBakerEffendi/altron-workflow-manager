...
public class CompanyResource {
	 ...
	 *         company, or with status 400 (Bad Request) if the company has already
	 *         an ID
	...
	public ResponseEntity<Company> updateCompany(@Valid @RequestBody Company company) throws URISyntaxException {
		...
		Company result = companyRepository.save(company);
		return ResponseEntity.ok().headers(HeaderUtil.createEntityUpdateAlert(ENTITY_NAME, company.getId().toString()))
				.body(result);
	}
	 ...
	 * @return the ResponseEntity with status 200 (OK) and with body the company, or
	...
	/**
	 * GET /company/get-:name : get company entity by company name
	 * 
	 * @param name
	 *            company name
	 * @return company entity
	 */
	@GetMapping("/company/get-{name}")
	public ResponseEntity<Company> getCompanyByName(@PathVariable String name) {
		log.debug("REST request to get Company : '{}'", name);
		return new ResponseEntity<>(companyService.getCompanyByName(name), null, HttpStatus.OK);
	}
}
