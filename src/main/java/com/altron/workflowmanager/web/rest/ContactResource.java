...
public class ContactResource {
	 ...
	 *         contact, or with status 400 (Bad Request) if the contact has already
	 *         an ID
	 ...
	 *         contact, or with status 400 (Bad Request) if the contact is not
	 *         valid, or with status 500 (Internal Server Error) if the contact
	 *         couldn't be updated
	 * @throws URISyntaxException
	 *             if the Location URI syntax is incorrect
	 */
	...
	public ResponseEntity<Contact> updateContact(@Valid @RequestBody Contact contact) throws URISyntaxException {
		...
		Contact result = contactRepository.save(contact);
		return ResponseEntity.ok().headers(HeaderUtil.createEntityUpdateAlert(ENTITY_NAME, contact.getId().toString()))
				.body(result);
	}
	 ...
	 * @return the ResponseEntity with status 200 (OK) and with body the contact, or
